...
def rpe_multi_head_attention_forward(
    query: Tensor,  # torch.Size([648, 2, 256])
    key: Tensor,  # ==query,torch.Size([648, 2, 256])
    value: Tensor,  # torch.Size([648, 2, 256])
    embed_dim_to_check: int,  # 256
    num_heads: int,  # 8
    in_proj_weight: Tensor,  # torch.Size([768, 256])
    in_proj_bias: Tensor,  # 768
    bias_k: Optional[Tensor],  # None
    bias_v: Optional[Tensor],  # None
    add_zero_attn: bool,  # False
    dropout_p: float,  # 0.1
    out_proj_weight: Tensor,  # torch.Size([256, 256])
    out_proj_bias: Tensor,  # 256
    training: bool = True,
    key_padding_mask: Optional[Tensor] = None,  # torch.Size([2, 648])
    ...
    # 2D-RPE
    rpe_q=None,
    rpe_k=None,  # iRPE(head_dim=32, num_heads=1, mode="contextual", method=3, transposed=True, num_buckets=81, initializer=<function iRPE.__init__.<locals>.initializer at 0x7f90d4f5f3a0>, rpe_config={'shared_head': True, 'mode': 'contextual', 'method': 3, 'alpha': 2.0, 'beta': 4.0, 'gamma': 16.0, 'num_buckets': 81})
    rpe_v=None,
    hw=None,  # (24, 27)
    ...
    if not use_separate_proj_weight:
        if (query is key or torch.equal(query, key)) and (key is value or torch.equal(key, value)):
            ...
            q, k, v = linear(query, in_proj_weight,
                             in_proj_bias).chunk(3, dim=-1)  # 输入,权重,偏差
            ...
            # This is inline in_proj function with in_proj_weight and in_proj_bias
            _b = in_proj_bias # Tensor:768
            _start = 0
            _end = embed_dim  # int 256
            _w = in_proj_weight[_start:_end, :]  # torch.Size([256, 256])
            if _b is not None:
                _b = _b[_start:_end]  # # Tensor:256
            q = linear(query, _w, _b)  # torch.Size([648, 2, 256])单纯线性变换一次,qkv各使用in_proj_bias(b)和in_proj_weight(w)的三分之一参数
            ...
            if _b is not None:
                _b = _b[_start:_end]
            k = linear(key, _w, _b)  # torch.Size([648, 2, 256])
            ...
            if _b is not None:
                _b = _b[_start:]
            v = linear(value, _w, _b)  # torch.Size([648, 2, 256])
    else:
    ...
    q = q.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)  # torch.Size([648, 2, 256])->torch.Size([16, 648, 32])
    if k is not None:
        k = k.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1)  # torch.Size([16, 648, 32])
    if v is not None:
        v = v.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1)  # torch.Size([16, 648, 32])
    ...
    '''
    q = q.contiguous()  # torch.Size([179968, 1, 4])
    k = k.contiguous()  # torch.Size([179968, 48, 4])

    attn_output_weights = torch.bmm(q, k.transpose(1, 2))  # torch.Size([179968, 1, 48])
    attn_output_weights_view = attn_output_weights.view(
        bsz, num_heads, *attn_output_weights.shape[-2:])  # view之后的改动会传回view之前，view只是观测上的区别

    # 2D relative position encoding on keys
    if rpe_k is not None:
        q_view = q.view(bsz, num_heads, tgt_len, head_dim)  # torch.Size([44992, 4, 1, 4])
        attn_output_weights_view += rpe_k(q_view, height=hw[0], width=hw[1])  # torch.Size([2, 8, 648, 32])+torch.Size([2, 8, 648, 648])=torch.Size([2, 8, 648, 648])  查询numpy的数组广播机制,将32重复至648再相加
